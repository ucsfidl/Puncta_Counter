...
public class PunctaCounterAutodetect extends JFrame implements ActionListener, ItemListener {
    ...
    public PunctaCntrImageCanvasAutodetect activeIC;
	 private PunctaCntrImageCanvasAutodetect ic1;
	 private PunctaCntrImageCanvasAutodetect ic2;
	 private int nextCanvasID = 1;
    
    private boolean isJava14;
    
    static PunctaCounterAutodetect instance;
    ...
    private boolean showNumbers = true;
	 ...
	 public PunctaCntrImageCanvasAutodetect compareTo = null;
...
//	public int PSD95orSynt;
    ...
    public PunctaCounterAutodetect(){
        ...
        //Autorun code starts here
          ...
          MinRingRatio = 2;  // NoteForUser: Minimum intensity ratio of the detected puncta, set lower (to 1.1 instead of 2) so to detect fainter puncta
          ArbitraryLocalBoundaryCutoff = 10; // NoteForUser: Minimum intensity ratio of background noise, set lower (to 2 instead of 10) to detect fainter puncta
        ...
        autoDetect(1); //same as autoDetectButton() for one image
        measure();
        String filePath = myDirectory +activeImg.getTitle()+"_1.xml";
        exportMarkers2(filePath);
        //End autorun code
    }
...
//-----------------------------INITIALIZE STACK(S)  
    private void initializeImage() {
      ...
      if (activeImg==null){
		...
		activeIC = new PunctaCntrImageCanvasAutodetect(activeImg,this,nextCanvasID,displayList);
		if (nextCanvasID == 1) {
			img1 = activeImg;
			ic1 = activeIC;
		   ...
		   BackgroundGridSizeAutoDetect = (int)Math.round(Math.sqrt(3.125*(PixelsX/MicronsX)*3.125*(PixelsY/MicronsY))); //sampling is 3.125 microns per pixel, equals 16 for 1024 pixel, 1x 200 micron image
...
//-----------------------------LOAD MARKERS
    public void loadMarkers(){  //PAINTING NOT SHOWING UP!!!!
      if (!compareMode) {  //This code runs when working with one image
		  String filePath = getFilePath(new JFrame(), "Marker File For "+activeImg.getTitle(), OPEN);
        ...
        if (storedfilename.equals(activeImg.getTitle())){
            //Vector<PunctaCntrMarkerVector> typeVector = rxml.readMarkerData();
		  ...
		  String filePath1 = getFilePath(new JFrame(), "Marker File For "+img1.getTitle(), OPEN);
        ...
        String filePath2 = getFilePath(new JFrame(), "Marker File For "+img2.getTitle(), OPEN);
...
//-----------------------------EXPORT MARKERS    
    public void exportMarkers2(String filePath) {
      //For one image only!!
    ...
    private String getFilePath(JFrame parent, String dialogMessage, int dialogType){
        ...
        switch(dialogType){
            case(SAVE):
                ...
                fd.setFile("PunctaCounter_"+filename.substring(0,filename.lastIndexOf("."))+"_1.xml");
                break;
        }
        fd.setVisible(true);
...
//-----------------------------AUTO DETECT PUNCTA FOR 1 OR 2 STACKS	    
	 private void autoDetect(int canvasID) {
			...
			for (int ii = 0; ii < PixelsX; ii += BackgroundGridSizeAutoDetect/2) {
				...
				if ((hereavg - bkgrnd_avg[z]) > bkgrnd_stddev[z]) {
					...
					if (besti < BackgroundGridSizeAutoDetect-2 && bestj < BackgroundGridSizeAutoDetect-2) {
						PunctaCntrMarker m = findBoundary(ii+besti, jj+bestj, z);
						if (m.getRad() > 0) {
		...
		//Now that all the markers have been added, properly group them and remove inadequate groups where z=1 or z>4
		...
		GroupAndRemoveRedundant();
		
		//these two lines must be commented out for automated autodetect to export xml files 
		//activeIC.repaint();
		//populateTxtFields();
	 }
