/* PUNCTA COUNTER ORIGINAL/MANUAL
 *
...
public class PunctaCounter extends JFrame implements ActionListener, ItemListener {
    ...
    public PunctaCntrImageCanvas activeIC;
	private PunctaCntrImageCanvas ic1;
	private PunctaCntrImageCanvas ic2;
	private int nextCanvasID = 1;
    
    private boolean isJava14;
    
    static PunctaCounter instance;
	...
	public PunctaCntrImageCanvas compareTo = null;
	...
	public int LinkedXML=0, Blur=0, DayXML1, DayXML2, GorRXML, ManualXML, LoadXML1, LoadXML2, WhichImage, DayXML3;
    ...
    public PunctaCounter(){
        ...
        IJ.showMessage(myDirectory);
        createPunctaCounterGUI();
...
//-----------------------------CREATES PANELS FOR COUNTER AND ACTION BUTTONS
	...
	private void createAutodetectGUI(){ // create the second GUI for user to input parameters 
      	...
      	//16 pixels for 1024x1024 1x image, 16 for 512x512 2x image, 32 for 512x512 4x image, 32 for 1024x1024 2x image
      	BackgroundGridSizeAutoDetect = (int)(Math.round(Math.sqrt(3.125*(PixelsX/MicronsX)*3.125*(PixelsY/MicronsY))));
	 ...
	 private void createLoadMarkersGUI2(){ // create the second GUI for user to input parameters 
         ...
         if (LinkedXML == 0) {
	         ...
	         gd.addNumericField("Load Red (1) or Green (2) XML:", 2,0);
	      	...
	      	if (!compareMode) {
				...
				if (!compareMode) {
				gd.addNumericField("Is this Img1 or Img 2:", 1,0); 
		...
		if (LinkedXML == 0) {
			if (!compareMode) {
				...
				if (!compareMode) {
				WhichImage = (int)gd.getNextNumber();
...
//-----------------------------INITIALIZE STACK(S)      
    private void initializeImage() {	
      ...
      if (activeImg==null){
		...
		activeIC = new PunctaCntrImageCanvas(activeImg,this,nextCanvasID,displayList);
         ...
         picknumber = new int[PixelsImg1Z+1]; //it is +1 because array starts at zero but we have bz starting at 1          		
	      ...
	      ImageStack stack = img2.getStack();
			...
			for (int bz = 1; bz <= PixelsImg2Z; bz++) {
				...
				MeasureData MDsignal2 = findSignalLevel2(ip, bz, signal_picknumber1[bz]);
   	...
   	autogroupButton.setEnabled(true);
   	removeinadequateButton.setEnabled(true);
   	addButton.setEnabled(true);
   	removeButton.setEnabled(true);
   	resetButton.setEnabled(true);
   	measureButton.setEnabled(true);
		resultsButton.setEnabled(true);
		manualgroupCheck.setEnabled(true);
		removeedgesButton.setEnabled(true);
   	setType("1");
      
		if (nextCanvasID == 3) {
			autolinkButton.setEnabled(true);
			unlinkCheck.setEnabled(true);
         manuallinkCheck.setEnabled(true);	
	   }
    }
          ...
          //  }
			...
			if (LinkedXML == 0) {
	            if (ManualXML == 0 || LoadXML1 == 0) {
	            	...
	            	loadMarkers2(filePath, filePath); //loadmarkers2 does not actually use filePath2 when not in compare mode
	            }    
	       ...
	       if (ManualXML == 0) {
		            	...
		            	//relinkBasedOnResultNums();
		             	...
		             	String filePath = myDirectory+"Day"+DayXML1+"Day"+DayXML2+fileSeparator+img1.getTitle()+"_Day"+DayXML3+".xml";
            	...
            	if (LinkedXML == 0) {         	
	 	            if (ManualXML == 0 || LoadXML1 == 0 || LoadXML2 == 0) {
		            	loadMarkers();
		             	...
		             	String filePath = myDirectory+img1.getTitle()+"_"+LoadXML1+".xml"; 
		             	String filePath2 = myDirectory+img2.getTitle()+"_"+LoadXML2+".xml";
	            	...
	            	if (ManualXML == 0) {
		            	...
		            	relinkBasedOnResultNums();
		             	...
		             	String filePath = myDirectory + "Day"+DayXML1+"Day"+DayXML2+fileSeparator+img1.getTitle()+"_Day"+DayXML2+".xml";
		             	String filePath2 = myDirectory + "Day"+DayXML1+"Day"+DayXML2+fileSeparator+img2.getTitle()+"_Day"+DayXML1+".xml";
		            	loadMarkers2(filePath, filePath2);
		            	//relinkBasedOnResultNums();
		            }            
	            }
...
//-----------------------------MEASURE BACKGROUND		
	...
	private MeasureData findSignalLevel(ImageProcessor ip, int z, int [] picknumber) { //finds average signal level per slices excluding cell bodies 
		...
		picknumber[z] = i - (int)Math.round(0.50*i); //one picknumber per z-section
	...
	private MeasureData findSignalLevel2(ImageProcessor ip, int z, int picknumber2) { //finds average signal level per slices excluding cell bodies 
		...
		if (i > 100 && i > picknumber2 && picknumber2 > 0) {								
			int[] signalarray2;
			...
			//IJ.log("img2 z = "+z+" red signal = "+signalarray2[count]); 
		 
			addSampleSignal2(MDsignal2, picknumber2, signalarray2[count], max, min, median, avg, stddev);
...
//-----------------------------MANUAL DETECT PUNCTA IN 1 OR 2 STACKS
	...
	public void MeasureByClick_old(int x, int y, int z, int canvasID) { //not in use 
		...
		ArbitraryLocalBoundaryCutoff=10; //this is set lower (to 2 instead of 10) so that when you can detect fainter puncta	
	...
	public void MeasureByClick_new(int x, int y, int z, int canvasID) {
		MinRingRatio=1.1;	 // NoteForUser:this is set lower (to 1.1 instead of 2) so that when you can detect fainter puncta																						
		ArbitraryLocalBoundaryCutoff=2; // NoteForUser: this is set lower (to 2 instead of 10) so that when you can detect fainter puncta	
    ...
    public void autoDetectButton() {
	 ...
	 private void autoDetect(int canvasID) {
		...
		for (int z = 1; z <= activeImg.getStackSize(); z++) {
            ...
            IJ.showStatus("Checkpoint:ID="+canvasID+"check2.6");
			ip = stack.getProcessor(z);
            IJ.showStatus("Checkpoint:ID="+canvasID+"check2.7");
			ip.resetRoi();
            IJ.showStatus("Checkpoint:ID="+canvasID+"check2.8");
			ip = ip.crop();

            IJ.showStatus("Checkpoint:ID="+canvasID+"check2.9");
			
								
			MeasureData MD = findBackgroundLevel(ip);
            IJ.showStatus("Checkpoint:ID="+canvasID+"check2.91");
            bkgrnd_avg[z] = MD.avg;
            bkgrnd_stddev[z] = MD.stddev;
            ...
            IJ.showStatus("Checkpoint:ID="+canvasID+"check3.0");
            ...
            IJ.showStatus("Checkpoint:ID="+canvasID+"check3.1");
            ...
            IJ.showStatus("Checkpoint:ID="+canvasID+"check3.2");
                    ...
                    IJ.showStatus("Checkpoint:ID="+canvasID+"check4");

                    ip = activeImg.getStack().getProcessor(z);
					ip.resetRoi();
					ip = ip.crop();	
				}
			}
            IJ.showStatus("Checkpoint:ID="+canvasID+"check5");

            if (canvasID == 1) {
                bkgrnd_avg1[z] = bkgrnd_avg[z];
                bkgrnd_stddev1[z] = bkgrnd_stddev[z];
            } else {
                bkgrnd_avg2[z] = bkgrnd_avg[z];
                bkgrnd_stddev2[z] = bkgrnd_stddev[z];
	...
	public void BestShiftEstimate(){
	 ...
	 public void autolink2(){
      ...
      for (int n = 0; n < PCMVB.size(); n++)
		...
		for (int n = 0; n < PCMVA.size(); n++) { //compares unlinked punctum in canvas A to undetected puncta in canvas B
			boolean pairing_found = false;
			PunctaCntrMarker mA = (PunctaCntrMarker)PCMVA.get(n);
			if (mA.isOwner()) { 
				for (int n2 = 0; n2 < PCMVB.size() && !pairing_found; n2++) {
      ...
      for (int n = 0; n < PCMVB.size(); n++) //AFTER being checked, reset to 0
		...
		//IJ.log("3916");
		GroupAndRemoveRedundant(canvasID); //if sequential with z plane puncta do only remove and use autogroupmarker?			
		//IJ.log("3934");	
		removeinadequate2(canvasID);
		//IJ.log("3936");
		autolink(); 
...
//-----------------------------MANUAL UNLINK AND RELINK
   ...
   //currently autogroupmarker is the same for psd95 and synt 10/19/2011 
   		...
   		if (canvasID == 1) { 
			   IJ.showStatus("Processing autogroup in image "+canvasID);
				...
				for (int n1 = 0; n1 < PCMVA.size(); n1++) {
					...
					BestMatch = 1000;
					BestMatch2 = 1000;
					IJ.showStatus("Processing autogroup in slice "+m1.getZ()+"/"+activeImg.getStackSize());
				}
			} else {
				IJ.showStatus("Processing autogroup in image "+canvasID);
				PunctaCntrMarkerVector PCMVB = (PunctaCntrMarkerVector)typeVector.get(1);
		...
		IJ.showStatus("Done processing autogroup");
	}	
